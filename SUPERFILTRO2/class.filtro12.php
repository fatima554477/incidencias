...
if ($sWhere2 != "") {
    // Quita el último 'OR' sobrante
    $sWhere22 = rtrim($sWhere2);
    $sWhere22 = preg_replace('/\s+OR\s*$/i', '', $sWhere22);

    $sWhere3 = " WHERE SUBSTRING_INDEX({$tables}.DOCUMENTO_INCIDENCIAS, '^^', 1) = '".$idem."' AND (".$sWhere22.") ";
} else {
    $sWhere3 = " WHERE SUBSTRING_INDEX({$tables}.DOCUMENTO_INCIDENCIAS, '^^', 1) = '".$idem."' ";
}
